...
void DXRSetup::initialise()
{
	...
	CreateIMGUIBuffer();
	UpdateIMGUIBuffer(1, 0.3f, 0.2f, 0, 1, 1, 5);
...
void DXRSetup::CreateRaytracingPipeline() 
{
	...
	pipeline.AddLibrary(context->m_hitLibrary.Get(), { L"ClosestHit", L"PlaneClosestHit", L"ShadowClosestHit"});
	...
	pipeline.AddRootSignatureAssociation(context->m_hitSignature.Get(), { L"HitGroup", L"PlaneHitGroup", L"ShadowHitGroup" });
	...
	//pipeline.SetMaxPayloadSize(32); // RGB + distance
	pipeline.SetMaxPayloadSize(16 + sizeof(unsigned int));
...
void DXRSetup::CreateShaderResourceHeap() 
{
	...
	// Increment past the acceleration structure descriptor to slot 2
	srvHandle.ptr += m_device->GetDescriptorHandleIncrementSize(
		D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
...
void DXRSetup::CreateCameraBuffer()
{
	...
	context->m_cameraBufferSize = 4 * sizeof(XMMATRIX);

	context->m_cameraBuffer = nv_helpers_dx12::CreateBuffer(
		m_device.Get(), context->m_cameraBufferSize,
...
void DXRSetup::UpdateCameraBuffer()
{
	...
	XMMATRIX perspective = XMMatrixPerspectiveFovLH(
		fovAngleY, m_app->GetAspectRatio(), 0.1f, 1000.0f);

	matrices[0] = XMMatrixInverse(nullptr, view);        // inverse view
	matrices[1] = XMMatrixInverse(nullptr, perspective); // inverse projection
...
void DXRSetup::UpdateIMGUIBuffer(int enableShadows, float shadowStrength, 
								float reflectionStrength, int textureIndex, 
								float colour, float enableSpecular, float lightPos)
{
	DXRContext* context = m_app->GetContext();
	...
	settings.specularValue = enableSpecular;
	settings.lightPos = lightPos;
